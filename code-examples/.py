...
class node:
    ...
    def __str__(self, unicode=True, spacing="  "):
        ...
        if self.right:  # right branch
            ret += self.right.__str__(
                spacing=spacing[:-2] + ("│ " if spacing[-2:] == "└─" else "  ") + "┌─",
                unicode=unicode,
            )
        ret += (
            spacing
            + (
                chr(self.value + (10121 if self.value < 11 else 9440))
                if unicode
                else str(self.value)
            )
            + "\n"
    ...
    async def async_sample1(self, unicode=True):
        """An example of an async coroutine"""
        await asyncio.sleep(1)
        return self.__str__(unicode=unicode)[:-1]
